...
namespace DarkWowSoft
{
    ...
    public partial class linkWindow : Window
    {
        ...
        #endregion
        #region Конструктор
        public linkWindow(IEnumerable<(int, string)> dataGridData, BitmapImage image)
        {
            InitializeComponent();
            this.Icon = new BitmapImage(new Uri("pack://application:,,,/DarkWowSoft;component/Resources/MainIcon.png"));
            #region Размер
            MaxHeight = MinHeight = 600; MaxWidth = MinWidth = 500;
            ...
            dataGrid = new System.Windows.Controls.DataGrid
            {
                ...
                //ToolTip = "Щелкните два раза, чтобы добавить параметр в правила формирования поисковых наборов",
            ...
            DataGridTextColumn commentColumn = new DataGridTextColumn { Width = 449, Header = "Комментарий", Binding = new System.Windows.Data.Binding("comment"), FontSize = 11, ElementStyle = style };
            dataGrid.Columns.Add(commentColumn);
            #endregion
            #region Заполнение DataGrid
            foreach ((int, string) data in dataGridData)
            {
                dataGrid.Items.Add(new DataGridItem() { id = data.Item1, comment = data.Item2 });
            }
            #endregion
            #endregion
